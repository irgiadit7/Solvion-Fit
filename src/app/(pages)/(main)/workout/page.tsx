'use client'; // Diperlukan untuk menggunakan useSession
...
// --- Komponen Utama Halaman Workout ---
export default function WorkoutPage() {
    const { data: session } = useSession(); // Mengambil data sesi pengguna
    const progress = 75; // Dummy progress
    const circleCircumference = 2 * Math.PI * 52;

    const userName = session?.user?.name?.split(' ')[0] || 'User'; // Mengambil nama depan pengguna
    ...
    return (
        <div className="max-w-5xl mx-auto space-y-10">
            ...
            <div className="bg-gradient-to-r from-blue-600 to-purple-600 rounded-2xl p-6 flex items-center justify-between shadow-lg">
                ...
                <button className="bg-white text-blue-600 font-semibold px-4 py-2 rounded-lg hover:bg-zinc-200 transition">
                    View
                </button>
            </div>

            {/* 3. Progress & Quick Start Section */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                ...
                <div className="md:col-span-2 bg-zinc-800 rounded-2xl p-6">
                     ...
                     <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                        <button className="flex flex-col items-center justify-center p-4 bg-zinc-700 rounded-lg hover:bg-zinc-600 transition group">
                            <FireIcon />
                            <span className="mt-2 text-sm font-semibold text-zinc-300 group-hover:text-white">5-min Warmup</span>
                        </button>
                         <button className="flex flex-col items-center justify-center p-4 bg-zinc-700 rounded-lg hover:bg-zinc-600 transition group">
                            <FireIcon />
                            <span className="mt-2 text-sm font-semibold text-zinc-300 group-hover:text-white">10-min Abs</span>
                        </button>
                         <button className="flex flex-col items-center justify-center p-4 bg-zinc-700 rounded-lg hover:bg-zinc-600 transition group">
                            <FireIcon />
                            <span className="mt-2 text-sm font-semibold text-zinc-300 group-hover:text-white">15-min Stretch</span>
                        </button>
                     </div>
                </div>
            </div>

            {/* 4. Workout Modules Section */}
            <div>
                <h3 className="text-xl font-bold mb-4">Explore Workouts</h3>
                <div className="relative mb-6">
                    <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                        <SearchIcon />
                    </div>
                    <input type="search" placeholder="Search for a workout, e.g., 'Full Body Strength'" className="w-full bg-zinc-800 text-white rounded-full py-3 pl-12 pr-4 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-shadow"/>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                    {[
                        { title: "Full Body Strength", desc: "Target all major muscle groups.", tag: "Beginner" },
                        { title: "HIIT Cardio Blast", desc: "High-intensity interval training.", tag: "Intermediate" },
                        { title: "Core & Abs Crusher", desc: "Strengthen your core muscles.", tag: "All Levels" },
                    ].map((item, index) => (
                        <div key={index} className="bg-zinc-800 rounded-lg shadow-lg overflow-hidden group transform hover:-translate-y-1 transition-transform duration-300">
                            <div className="w-full h-40 bg-zinc-700 bg-cover bg-center group-hover:opacity-80 transition-opacity" style={{backgroundImage: "url('https://source.unsplash.com/random/400x300/?fitness,"+ index +"')"}}></div>
                            <div className="p-4">
                                <span className="inline-block bg-blue-500/20 text-blue-400 text-xs font-semibold px-2 py-1 rounded-full mb-2">{item.tag}</span>
