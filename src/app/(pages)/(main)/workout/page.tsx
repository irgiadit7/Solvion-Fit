...
import type { Category, WorkoutPlan } from '@prisma/client';
...
// Tipe baru untuk data WorkoutPlan yang menyertakan Category
type WorkoutPlanWithCategory = WorkoutPlan & {
  category: Category;
};
...
export default function WorkoutPage() {
    ...
    const [workoutPlans, setWorkoutPlans] = useState<WorkoutPlanWithCategory[]>([]);
    ...
    // Efek untuk mengambil data dari API saat komponen pertama kali dimuat
    useEffect(() => {
        ...
        <div className="max-w-5xl mx-auto space-y-10">
            ...
            <div>
                ...
                <div className="relative mb-6">
                    ...
                    <input type="search" placeholder="Search for a workout..." className="w-full bg-zinc-800 text-white rounded-full py-3 pl-12 pr-4 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-shadow"/>
                </div>

                {isLoading ? (
                    <p className="text-zinc-400 text-center">Loading workouts...</p>
                ) : (
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                        {workoutPlans.map((plan) => (
                            <div key={plan.id} className="bg-zinc-800 rounded-lg shadow-lg overflow-hidden group transform hover:-translate-y-1 transition-transform duration-300">
                                <div className="w-full h-40 bg-zinc-700 bg-cover bg-center group-hover:opacity-80 transition-opacity" style={{backgroundImage: `url(${plan.coverImageUrl || 'https://source.unsplash.com/random/400x300/?fitness'})`}}></div>
                                <div className="p-4">
                                    <span className="inline-block bg-blue-500/20 text-blue-400 text-xs font-semibold px-2 py-1 rounded-full mb-2">{plan.difficulty}</span>
